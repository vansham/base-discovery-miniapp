    ...
    <main className="min-h-screen bg-gradient-to-b from-black to-zinc-900 text-white">
      ...
      <div className="flex justify-between items-center p-6 border-b border-zinc-800">
        <div>
          ...
          <p className="text-sm text-zinc-400">Smart Liquidity + New Pools + Signals</p>
        </div>
      ...
      {loading && (
        <div className="p-6 text-zinc-400">Loading Base Alpha Data...</div>
      )}
  ...
  <div className="grid md:grid-cols-2 gap-4">
    {newPools.map((p,i)=>(
      ...
      <Section title="Smart Liquidity Signals">
        <div className="grid md:grid-cols-2 gap-4">
          {whales.map((w,i)=>(
            <Card key={i} pair={`${w.baseToken?.symbol}/${w.quoteToken?.symbol}`} liq={w.liquidity?.usd} />
          ))}
        </div>
      </Section>
